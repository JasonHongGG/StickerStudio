      ...
      for (let y = 0; y < h; y++) {
        for (let x = 0; x < w; x++) {
          ...
          if (mask[idx] !== 1) {
            ...
            // White Edge Protection logic:
            // High brightness, low saturation check is implicitly handled because isGreenPixel would fail.
            // But we need to make sure we didn't delete it? 
            // The flood fill logic stops at non-green. 
            // If the white stroke has a greenish tint, it might have been eaten?
            // The requirement says: "Detect 1-3px outer area... force keep". 
            // Since we rely on Flood Fill stopping, we are safer. 
            // However, let's implement Spill Suppression on the boundary.
